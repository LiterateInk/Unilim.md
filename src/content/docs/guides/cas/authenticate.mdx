...
<Tabs>
  ...
  <TabItem label=".rs">
    ```rust
    use unilim::cas::{ CAS, PendingAuth };
    let auth: PendingAuth = CAS::initialize("username", "password").await?;
    ```
  </TabItem>
  <TabItem label=".swift">
    ```swift
    import UnilimCAS
    var auth: PendingAuth = try await CAS.initialize(username: "username", password: "password")
    ```
  </TabItem>
</Tabs>

Once a session has been initialized, you'll end up with a `PendingAuth` instance.
...
You'll be asked to resolve a 2FA challenge when `solved` is falsy on `PendingAuth`.
...
<Tabs>
  ...
  <TabItem label=".swift">
    ```swift
    try await auth.solveWithTotp(totp: "123456")
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label=".swift">
    ```swift
    try await auth.sendEmailCode()
    try await auth.solveWithEmailCode(code: "123456")
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label=".rs">
    ```rust
    let cas: CAS = auth.finish().await?;
    ```
  </TabItem>
  <TabItem label=".swift">
    ```swift
    let cas: CAS = try await auth.finish()
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label=".swift">
    ```swift
    let cas: CAS = try await CAS.restore(
      username: "username", password: "password",  // <- the usual you'd give in initialize()
      llngconnection: llngconnection, key: key     // <- the variables we retrieved
    )
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label=".swift">
    ```swift
    let cas: CAS = CAS.temporary(cookie: "value")
    ```
  </TabItem>
</Tabs>
